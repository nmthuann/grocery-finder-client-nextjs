...
export const NavbarMobileComponent: React.FC<NavbarMobileComponentProps> = ({ location, categories }) => {
  ...
  return (
    <Navbar isBordered>
        ...
        <NavbarBrand>
          <Link href='/' className='flex items-center'>
            <h1 className='font-Inter text-xs cursor-pointer '>
              Tạp Hóa <span className='text-[#64ff4c] ml-1 font-semibold'>Tân Hiệp</span>
            </h1>
          </Link>
        </NavbarBrand>
      </NavbarContent>
      ...
      <NavbarContent justify='end' className='gap-2'>
        <Badge content={cart.items.length} shape='circle' color='danger' className='dark:border-white border-slate-900'>
          <Button
            radius='full'
            isIconOnly
          ...
          >
            <ShoppingCart size={18} className='text-slate-800 dark:text-slate-400' />
          </Button>
        </Badge>
        <SearchDialog />
          ...
          <div>
            <Tooltip content='Đăng nhập/ Đăng ký' placement='bottom'>
              <Button
                ...
                aria-label='Login'
              >
                <User2Icon className='text-slate-800 dark:text-slate-400' size={18} />
              </Button>
            </Tooltip>

            <AuthenticationModal isOpen={isOpen} onOpenChange={onOpenChange} location={location} size='xs' />
          </div>
        )}
        <ThemeToggle />
      </NavbarContent>
