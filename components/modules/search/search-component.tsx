...
const SearchComponent = () => {
  ...
  return (
    <div>
      <Button onPress={onOpen} color='default' variant='bordered' startContent={<SearchIcon size={20} />}>
        Tìm kiếm...
      </Button>
      ...
      <Modal isOpen={isOpen} onOpenChange={onOpenChange} placement='center'>
        <ModalContent>
          {onClose => (
            <>
              <ModalHeader className='flex flex-col gap-1'>Tìm kiếm bất cứ sản phẩm gì bạn muốn</ModalHeader>
              <ModalBody>
                <Input
                  isClearable
                  radius='lg'
                    ...
                    <div>
                      <SearchListBoxWrapper>
                        <Listbox
                          ...
                          emptyContent={' Vui lòng nhập vào ô tìm kiếm'}
                        >
                          {item => (
                            <ListboxItem
                              key={item.id}
                              href={`${item.categoryUrl}${item.slug}`}
                              endContent={<ChevronRight className='w-4 h-4 text-slate-600' />}
                            >
