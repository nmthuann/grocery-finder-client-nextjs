...
const ResendOTPCountdown: React.FC<ResendOTPCountdownProps> = ({ resendTimeout, emailToResend }) => {
    ...
    <div>
      {resending ? (
        <div className='flex items-center justify-center space-x-0'>
          <h3 className=' text-sm '>Gửi lại OTP trong {formattedCountdown}</h3>
          {onBtnLoad ? (
        ...
        <h3 className=' mr-8 text-sm text-primary'>Gửi lại OTP trong {formattedCountdown}</h3>
      )}
    </div>
  )
}
