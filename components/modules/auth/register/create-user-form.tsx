...
export const CreateUserForm: React.FC<CreateUserFormProps> = ({
  ...
  const handleCityChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedDistricts = location.find((city: Location) => city.Id === selectedCityId)
  ...
  async function onSubmit(values: z.infer<typeof CreateUserFormSchema>) {
    try {
      ...
      } else if (res.data.message === AuthExceptionMessages.REGISTER_USER_FAILED) {
        setOnBtnLoad(false)
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-3'>
              ...
              <FormItem>
                <FormControl>
                  <InputNextUI
                    ...
                    isInvalid={isInvalid}
                    errorMessage={isInvalid ? form.formState.errors.address?.message : ''}
                    onValueChange={field.onChange}
                  />
                </FormControl>
              </FormItem>
        ...
        <p className='text-center text-small'>
          Bạn đã có tài khoản?{' '}
          <LinkNextUI size='sm' onPress={() => setSelected('login')} underline='hover' className='cursor-pointer'>
            Đăng nhập
          </LinkNextUI>
        </p>
      </form>
