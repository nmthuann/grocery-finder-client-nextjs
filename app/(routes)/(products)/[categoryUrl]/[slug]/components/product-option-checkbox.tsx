...
export const ProductOptionCheckbox: React.FC<ProductOptionCheckboxProps> = ({ option, isSelected, onOptionChange }) => {
  ...
  const discountPercentage = option.importPrice
    ? Math.round(((option.importPrice - option.salePrice) / option.importPrice) * 100)
    : null
  return (
    <div>
      <Card isFooterBlurred radius='lg' className='border-none'>
        <Image alt={option.productName} className='object-cover' height={200} src={option.productThumb} width={200} />
        <CardFooter
          className=' 
                    justify-between 
                    ...
                    '
        >
          <p className='text-small font-bold text-slate-900'>{truncatedSkuName}</p>
        </CardFooter>
      </Card>
      <div className='flex flex-col gap-2 text-small mt-2'>
        <Checkbox isSelected={isSelected} onValueChange={onOptionChange} color='primary'>
          {<Currency value={option.salePrice.toString()} />}
        </Checkbox>
        {option.importPrice > option.salePrice && (
          <div className='flex items-center gap-2'>
            <span className='text-tiny text-default-400 line-through'>{option.importPrice} đ</span>
