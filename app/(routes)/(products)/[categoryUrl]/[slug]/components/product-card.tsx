...
const ProductCard: React.FC<ProductCardProps> = ({ data }) => {
    ...
    const discountPercentage = data.importPrice
        ? Math.round(
              ((data.importPrice - data.salePrice) / data.salePrice) * 100
          )
        : null;
    return (
        <Card
            ...
            <CardFooter className="flex flex-col justify-between h-full">
                ...
                <div className="flex w-full">
                    <Chip
                        ...
                        className={`${
                            data.isActive
                                ? "dark:text-[#64FF4B] text-green-500 text-sm"
                ...
                <div className="flex flex-row items-baseline justify-between w-full mt-2">
                    ...
                    <div className="flex justify-end ">
                        <p className="text-tiny text-right text-slate-700 dark:text-slate-300">
                            {`Đã bán ${data.sold}`}
                        </p>
                    </div>
                </div>
